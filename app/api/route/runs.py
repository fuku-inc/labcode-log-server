from define_db.models import Run, Project, Protocol, User, Operation, Process
...
def create(
    ...
    with SessionLocal() as session:
        ...
        if not protocol:
            raise HTTPException(status_code=400, detail=f"Protocol with id {protocol_id} not found")
...
def update(id: int, project_id: int = Form(), protocol_id: int = Form(), user_id: int = Form(), storage_address: str = Form()):
    with SessionLocal() as session:
        run = session.query(Run).filter(Run.id == id).first()
        # Check run existence
        if not run:
...
def patch(id: int, attribute: str = Form(), new_value: str = Form()):
    with SessionLocal() as session:
        ...
        match attribute:
            ...
            case "protocol_id":
                protocol = session.query(Protocol).filter(Protocol.id == new_value).first()
                if not protocol:
                    raise HTTPException(status_code=400, detail=f"Protocol with id {new_value} not found")
                run.protocol_id = new_value
            case "user_id":
